...
export default function MatchSuggestionsPanel({ postId }) {
  ...
  if (isLoading) {
    return (
      <div className="space-y-3 mt-8">
        ...
        <div className="animate-pulse bg-slate-100 rounded-2xl h-32 w-full"></div>
        <div className="animate-pulse bg-slate-100 rounded-2xl h-32 w-full"></div>
        <div className="animate-pulse bg-slate-100 rounded-2xl h-32 w-full"></div>
      </div>
    );
  ...
  if (!matches?.length) {
    return (
      <div className="text-center py-8 text-slate-400 bg-slate-50 border border-slate-100 rounded-2xl mt-8">
        <p className="font-medium text-slate-500">No matches found yet.</p>
