            ...
            <div className="hidden md:flex space-x-6">
              ...
              {isAuthenticated && !isAdmin && (
                <>
                  ...
                  <Link to="/heatmap" className="text-gray-600 hover:text-indigo-600 font-medium transition">
                    🗺️ Heatmap
                  </Link>
                </>
              )}
              {isAdmin && (
                <>
                  <Link to="/admin" className="text-indigo-600 hover:text-indigo-800 font-medium transition">
                    Admin Panel
                  </Link>
            ...
            {isAdmin && (
              <>
                <Link
                  to="/admin"
                  onClick={() => setMobileMenuOpen(false)}
                  className="block px-3 py-2 text-base font-medium text-indigo-600 hover:bg-indigo-50 rounded-md transition"
                >
                  Admin Panel
                </Link>
                <Link
                  to="/admin/heatmap"
                  onClick={() => setMobileMenuOpen(false)}
                  className="block px-3 py-2 text-base font-medium text-orange-600 hover:bg-orange-50 rounded-md transition"
                >
                  🗺️ Heatmap
                </Link>
              </>
            ...
            {!isAuthenticated && (
              <div className="border-t border-gray-100 mt-2 pt-2">
                ...
                <Link
                  to="/register"
                  onClick={() => setMobileMenuOpen(false)}
                  className="block w-full text-center px-4 py-2 mt-2 text-base font-medium text-indigo-600 border border-indigo-600 rounded-lg hover:bg-indigo-50 transition"
                >
                  Register
                </Link>
              </div>
